...
const MeetingDetail = () => {
  ...
  const fetchMeetingDetail = async () => {
    try {
      ...
      // Combine AI insights from recordings with manual insights
      const keyPointsArray: { text: string; source: 'ai' | 'user' }[] = [];
      const decisionsArray: { text: string; source: 'ai' | 'user' }[] = [];
      const tasksArray: { id: string; text: string; assignee?: string; deadline?: Date; source: 'ai' | 'user' }[] = [];

      // Extract AI insights from recordings
      data.recordings?.forEach((recording: Recording) => {
        console.log('[MEETING_DETAIL] Processing recording:', recording);
        console.log('[MEETING_DETAIL] Recording insights:', recording.insights);
        recording.insights?.forEach((insight: any) => {
          console.log('[MEETING_DETAIL] Processing insight:', insight);
          if (insight.type === 'insight' && insight.data) {
            const insightType = insight.data.insight_type;
            if (insightType === 'key_point' && insight.data.point) {
              console.log('[MEETING_DETAIL] Adding AI keyPoint:', insight.data.point);
              keyPointsArray.push({ text: insight.data.point, source: 'ai' });
            } else if (insightType === 'decision' && insight.data.decision) {
              console.log('[MEETING_DETAIL] Adding AI decision:', insight.data.decision);
              decisionsArray.push({ text: insight.data.decision, source: 'ai' });
            } else if (insightType === 'action_item' && insight.data.task) {
              console.log('[MEETING_DETAIL] Adding AI task:', insight.data.task);
              tasksArray.push({
              ...
              console.log('[MEETING_DETAIL] Unknown insight type or missing data:', insightType, insight.data);
            }
          } else {
            console.log('[MEETING_DETAIL] Insight does not match expected structure:', insight);
          }
        });
      });

      // Add manual insights if available
      if (data.manual_insights) {
        console.log('[MEETING_DETAIL] Manual insights received:', data.manual_insights);
        data.manual_insights.keyPoints?.forEach((point: any) => {
          console.log('[MEETING_DETAIL] Adding keyPoint:', point);
          keyPointsArray.push({ text: point.text, source: 'user' });
        });
        data.manual_insights.decisions?.forEach((decision: any) => {
          console.log('[MEETING_DETAIL] Adding decision:', decision);
          decisionsArray.push({ text: decision.text, source: 'user' });
        });
        data.manual_insights.tasks?.forEach((task: any) => {
          console.log('[MEETING_DETAIL] Adding task:', task);
          tasksArray.push({
            id: task.id || (Date.now() + Math.random()).toString(),
            text: task.text,
        ...
        console.log('[MEETING_DETAIL] No manual insights found in data');
      }
      ...
      // Process messages for transcription and translation panels
      const messagesArray: any[] = [];
      const translatedMessagesArray: any[] = [];

      data.recordings?.forEach((recording: Recording) => {
        recording.transcripts?.forEach((transcript: any) => {
          messagesArray.push({
            id: `${recording.recording_id}_${transcript.start}`,
            speaker: transcript.speaker_label,
            text: transcript.original_transcript,
            timestamp: transcript.start,
            language: transcript.detected_language || 'en'
          });

          translatedMessagesArray.push({
            id: `${recording.recording_id}_${transcript.start}`,
            speaker: transcript.speaker_label,
      ...
      setTranslatedMessages(translatedMessagesArray);
    } catch (error) {
  ...
  const handleExport = async () => {
    try {
      ...
      // Add logo at bottom center (temporarily disabled due to base64 image format issues)
  ...
  const messagesArray = recordings.flatMap(rec => 
  ...
  const translationsArray = recordings.flatMap(rec => 
    rec.transcripts.map(t => ({
      id: t.id || crypto.randomUUID(),
      speaker: t.speaker_label,
      speakerInitials: t.speaker_label.replace('SPEAKER_', 'S'),
